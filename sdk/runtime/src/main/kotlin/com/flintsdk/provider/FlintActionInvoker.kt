 ...
 */
internal object FlintActionInvoker {
    ...
    private fun findActionNode(
        ...
        for (node in nodes) {
            val nodeActionName = node.config.getOrNull(FlintSemantic.ActionName) ?: continue
            if (nodeActionName != actionName) continue

            if (listId != null && itemIndex != null) {
                val nodeItemIndex = node.config.getOrNull(FlintSemantic.ItemIndex) ?: findParentItemIndex(node)
    ...
    private fun findParentItemIndex(node: SemanticsNode): Int? {
        ...
        while (parent != null) {
            val idx = parent.config.getOrNull(FlintSemantic.ItemIndex)
    ...
    private fun findParentListId(node: SemanticsNode): String? {
        var parent = node.parent
        while (parent != null) {
            val id = parent.config.getOrNull(FlintSemantic.ListId)
