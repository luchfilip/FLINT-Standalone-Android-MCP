...
import dev.mcphub.acp.model.AcpScreenSnapshot
import kotlinx.serialization.json.Json
import java.util.concurrent.CountDownLatch
import java.util.concurrent.TimeUnit

/**
 * ContentProvider that exposes ACP capabilities to the Hub.
 *
 * Supports the following methods via call():
 * - get_schema: Returns the app's ACP schema JSON
 * - get_screen: Returns the current screen name
 * - read_screen: Returns structured snapshot of current screen
 * - call_tool: Invokes a registered ACP tool
...
class AcpProvider : ContentProvider() {
    ...
    private fun handleGetSchema(): Bundle {
        return try {
            val schemaClass = Class.forName("dev.mcphub.acp.generated.AcpSchemaHolder")
            ...
            Bundle().apply { putString("_error", "AcpSchemaHolder not found. Is KSP configured?") }
        }
    }

    private fun handleGetScreen(): Bundle {
        val screen = Acp.getScreenName()
        return Bundle().apply {
    ...
    private fun handleReadScreen(): Bundle {
        val snapshot = AcpTreeWalker.snapshot()
        val snapshotJson = json.encodeToString(AcpScreenSnapshot.serializer(), snapshot)
        return Bundle().apply {
    ...
    private fun handleCallTool(extras: Bundle?): Bundle {
        ...
        val params = extras.toAcpMap().filterKeys { !it.startsWith("_") }
        ...
        mainHandler.post {
            try {
                resultHolder[0] = Acp.routeTool(toolName, params)
            } finally {
                latch.countDown()
            }
        }
    ...
    private fun handleInvokeAction(extras: Bundle?): Bundle {
        ...
        mainHandler.post {
            try {
                success = AcpActionInvoker.invoke(actionName, listId, itemIndex)
            } finally {
                latch.countDown()
            }
        }
    ...
    // Required ContentProvider methods — unused for ACP
